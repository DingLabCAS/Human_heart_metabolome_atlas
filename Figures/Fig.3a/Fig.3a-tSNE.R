...
class_patt <- paste("*", class_order, collapse="_*|", sep="_")
...
res2$Group = groups
res2$Type = factor(type, levels=(c("D", "O")))

write.table(res2, "Fig.3a-tSNE.txt",sep = "\t", row.names = F, quote = F)


# P012 <- ggplot(res2, aes(x = V1, y = V2, fill = Type)) +   
...
# 
# 
# pdf("Fig.3a-tSNE.pdf", width = 12, height = 7.5)
# P012
# dev.off()


# save.image("Fig.3a-tSNE.RData")
